...
# === STEERING MOD (validity controller): structured per-constraint violation witnesses. ===
# Separate from adapted_validate_packing (which is the AUTHORITATIVE valid/invalid classifier and
# is left byte-identical). This does a FULL pass (no early return) and records EVERY overlap pair
# and boundary circle with its magnitude, for the lambda/validity controller's witnesses. Consumed
# only by the outer-loop steering controller; never shown to the model. Constraint families v1:
# "overlap" (pairwise non-overlap) and "boundary" (inside-unit-square).
def collect_violations(
    run_output: Tuple[np.ndarray, np.ndarray, float], atol: float = 0.0
) -> Dict[str, Any]:
    centers, radii, _ = run_output
    if not isinstance(centers, np.ndarray):
        centers = np.array(centers)
    if not isinstance(radii, np.ndarray):
        radii = np.array(radii)
    n_expected = 26
    out: Dict[str, Any] = {"overlap": [], "boundary": [], "malformed": False}
    if centers.shape != (n_expected, 2) or radii.shape != (n_expected,) \
            or not np.all(np.isfinite(centers)) or not np.all(np.isfinite(radii)):
        out["malformed"] = True
        return out
    # boundary: how far each circle pokes outside the unit square (max over the 4 sides)
    for i in range(n_expected):
        x, y = float(centers[i][0]), float(centers[i][1])
        r = float(radii[i])
        amt = max(0.0, -(x - r) - atol, (x + r) - 1.0 - atol,
                  -(y - r) - atol, (y + r) - 1.0 - atol)
        if amt > 0:
            out["boundary"].append({"i": i, "amount": round(amt, 6)})
    # overlap: penetration depth (r_i + r_j) - dist for every violating pair
    for i in range(n_expected):
        for j in range(i + 1, n_expected):
            dist = float(np.sqrt(np.sum((centers[i] - centers[j]) ** 2)))
            pen = (float(radii[i]) + float(radii[j])) - dist - atol
            if pen > 0:
                out["overlap"].append({"i": i, "j": j, "amount": round(pen, 6)})
    # sort witnesses most-severe first (validity controller prefers severe/recent)
    out["overlap"].sort(key=lambda d: -d["amount"])
    out["boundary"].sort(key=lambda d: -d["amount"])
    return out
# === END STEERING MOD (validity controller) ===
...
def aggregate_circle_packing_metrics(
    ...
    # The oracle score above (combined_score = reported_sum) is LEFT UNTOUCHED and remains the
    # recorded basis for all comparisons. Here we additionally record, in private_metrics:
    #   oracle_score, valid, and aux_scores = {name: value} for every aux.
    # These are consumed ONLY by the parent-selection bandit. They are deliberately NOT put
    # into `text_feedback` or `public` -- the model never sees them, so any effect is
    # attributable to WHICH PARENT WAS BRED, nothing else (pure selection intervention).
    # Wrapped so an aux failure can never break the evaluation loop; private_metrics is the
    # same object as metrics["private"], so mutating it updates the returned metrics in place.
    try:
        from aux_evals import compute_all

        valid, _ = adapted_validate_packing(results[0])
        private_metrics["oracle_score"] = float(reported_sum)
        private_metrics["valid"] = bool(valid)
        private_metrics["aux_scores"] = compute_all(centers, radii)
        # === STEERING MOD (validity controller): structured witnesses for lambda ===
        private_metrics["violations"] = collect_violations(results[0])
        # === END STEERING MOD ===
    except Exception as e:  # noqa: BLE001 - intentional: aux must never break the loop
        print(f"[ALG2 MOD] aux computation skipped due to error: {e}")
